...
$derivedState='/home/farmacia/.farmacia';
if(preg_match('#^/home/([^/]+)/htdocs/#',APP_ROOT,$m))$derivedState='/home/'.$m[1].'/.farmacia';
load_env_file($derivedState.'/.env');
if($derivedState!=='/home/superamplitude/.farmacia')load_env_file('/home/superamplitude/.farmacia/.env'); // compatibilidade de migração
load_env_file(APP_ROOT.'/.env');
function env(string $key,mixed $default=null):mixed{$v=getenv($key);return $v===false?$default:$v;}
function private_state_dir():string{global $derivedState;return rtrim((string)env('PRIVATE_STATE_DIR',$derivedState),'/');}
...
if(session_status()!==PHP_SESSION_ACTIVE){session_name('farmacia_session');session_set_cookie_params(['httponly'=>true,'secure'=>!empty($_SERVER['HTTPS']),'samesite'=>'Lax','path'=>env('APP_BASE','/')?:'/']);session_start();}
require APP_ROOT.'/src/Database.php';require APP_ROOT.'/src/Schema.php';require APP_ROOT.'/src/Auth.php';require APP_ROOT.'/src/Catalog.php';require APP_ROOT.'/src/Pharmacy.php';require APP_ROOT.'/src/AiAssistant.php';require APP_ROOT.'/src/R2Storage.php';
