...
create_php_pool(){
  ...
  while port_in_use "$port"; do
    port=$((port+1))
    [[ "$port" -le 19182 ]] || fail 'no free PHP-FPM port in reserved range'
  done
...
ensure_php_pool(){
  ...
  [[ -n "$found" ]] || found="$(create_php_pool)"
  pool="${found%%|*}"
  listen="${found#*|}"
  [[ -f "$pool" && -n "$listen" ]] || fail 'could not determine PHP-FPM pool'
  testbin="$(command -v php-fpm${PHP_VERSION} || command -v php-fpm8.2 || true)"
  if [[ -n "$testbin" ]]; then
    "$testbin" -t >/tmp/farmacia-php-fpm-test.out 2>&1 || { cat /tmp/farmacia-php-fpm-test.out >&2; fail 'php-fpm config test failed'; }
  fi
...
origin_http(){
  curl -k -sS --max-time 15 --resolve "${DOMAIN}:443:127.0.0.1" -o "$2" -w '%{http_code}' "$1" 2>/dev/null || true
}

repair_php_vhost(){
  validate_contract
  [[ -s "$APP_DIR/index.php" ]] || fail "front controller missing: ${APP_DIR}/index.php"
  php -l "$APP_DIR/index.php" >/dev/null || fail 'index.php failed PHP lint'

  local listen target ts backup tmp health admin i
...
src=os.environ['VHOST_SRC']
dst=os.environ['VHOST_DST']
target=os.environ['FASTCGI_TARGET']
...
def find_location_block(source, pattern):
...
# Convert the legacy Node proxy block if it still exists.
if 'proxy_pass http://127.0.0.1:3005/;' in text:
    block=find_location_block(text, r'^\s*location\s+/\s*\{')
    if not block:
        raise SystemExit('PROXY_LOCATION_NOT_FOUND')
    start,end=block
    replacement=f'''  location / {{\n    try_files $uri $uri/ /index.php?$query_string;\n  }}\n\n  location ~ \\.php$ {{\n    try_files $uri =404;\n    include fastcgi_params;\n    fastcgi_param SCRIPT_FILENAME $document_root$fastcgi_script_name;\n    fastcgi_param SCRIPT_NAME $fastcgi_script_name;\n    fastcgi_index index.php;\n    fastcgi_pass {target};\n  }}'''
    text=text[:start]+replacement+text[end:]
else:
    # Keep the current PHP location but refresh the upstream target.
    if re.search(r'(?m)^\s*fastcgi_pass\s+[^;]+;', text):
        text=re.sub(r'(?m)^(\s*fastcgi_pass\s+)[^;]+;', lambda m: m.group(1)+target+';', text)
    else:
        insert=f'''\n  location ~ \\.php$ {{\n    try_files $uri =404;\n    include fastcgi_params;\n    fastcgi_param SCRIPT_FILENAME $document_root$fastcgi_script_name;\n    fastcgi_param SCRIPT_NAME $fastcgi_script_name;\n    fastcgi_index index.php;\n    fastcgi_pass {target};\n  }}\n'''
        pos=text.rfind('}')
        if pos < 0:
            raise SystemExit('SERVER_BLOCK_END_NOT_FOUND')
        text=text[:pos]+insert+text[pos:]

# The observed 403 is caused by / resolving as the document-root directory.
# Force only the exact root request through index.php. Other static paths remain unchanged.
old_exact=find_location_block(text, r'^\s*location\s*=\s*/\s*\{')
if old_exact:
    text=text[:old_exact[0]]+text[old_exact[1]:]

root_dispatch=f'''  location = / {{\n    include fastcgi_params;\n    fastcgi_param SCRIPT_FILENAME $document_root/index.php;\n    fastcgi_param SCRIPT_NAME /index.php;\n    fastcgi_index index.php;\n    fastcgi_pass {target};\n  }}\n\n'''
regular=find_location_block(text, r'^\s*location\s+/\s*\{')
if regular:
    text=text[:regular[0]]+root_dispatch+text[regular[0]:]
else:
    php_block=find_location_block(text, r'^\s*location\s+~\s+\\\.php\$\s*\{')
    if not php_block:
        raise SystemExit('NO_INSERTION_POINT_FOR_ROOT_DISPATCH')
    text=text[:php_block[0]]+root_dispatch+text[php_block[0]:]
...
PY
  ...
  for i in $(seq 1 10); do
    health="$(origin_http "https://${DOMAIN}/?health=1" /tmp/farmacia-root-health.out)"
    [[ "$health" == 200 ]] && break
    sleep 1
  done
  admin="$(origin_http "https://${DOMAIN}/admin.php" /tmp/farmacia-admin-health.out)"

  if [[ "$health" != 200 || "$admin" != 200 ]]; then
    echo "VHOST_POSTCHECK_ROOT_HTTP=${health}" >&2
    echo "VHOST_POSTCHECK_ADMIN_HTTP=${admin}" >&2
    head -c 1200 /tmp/farmacia-root-health.out >&2 2>/dev/null || true
    cp -a "$backup" "$VHOST"
    nginx -t >/dev/null 2>&1 || true
    systemctl reload nginx || true
    fail "post-repair health failed; vhost restored from ${backup}"
  fi

  echo "VHOST_BACKUP=${backup}"
  echo "VHOST_MODE=php-fpm"
  echo "VHOST_FASTCGI_PASS=${target}"
  echo 'VHOST_ROOT_DISPATCH=index.php'
  echo "VHOST_ROOT_HTTP=${health}"
  echo "VHOST_ADMIN_HTTP=${admin}"
...
diagnose(){
  ...
  ls -l "$APP_DIR/index.php" 2>/dev/null || true
  php -l "$APP_DIR/index.php" 2>&1 || true
  echo '--- active vhost ---'
  sed -n '1,220p' "$VHOST" 2>/dev/null || true
  ...
  echo '--- origin health ---'
  local root_http admin_http public_http
  root_http="$(origin_http "https://${DOMAIN}/?health=1" /tmp/farmacia-origin-body)"
  admin_http="$(origin_http "https://${DOMAIN}/admin.php" /tmp/farmacia-origin-admin-body)"
  public_http="$(curl -k -sS -L --max-time 15 -o /tmp/farmacia-public-body -w '%{http_code}' "https://${DOMAIN}/?health=1" 2>/dev/null || true)"
  echo "DIAG_ORIGIN_HTTP=${root_http:-000}"
  echo "DIAG_ADMIN_HTTP=${admin_http:-000}"
  echo "DIAG_PUBLIC_HTTP=${public_http:-000}"
  ...
  [[ -n "$site_error" && -f "$site_error" ]] && tail -n 80 "$site_error" 2>/dev/null || true
}
